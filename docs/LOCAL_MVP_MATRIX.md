...
| F M11 packaging | Python/web images; Compose bootstrap/migration/runtime identities; V1 deployment and backup scripts. M12A adds `deploy/compose.homelab.yml`, `scripts/install-homelab.sh`, `scripts/provision_secrets.py`, `scripts/preflight.sh`, the `owner-bootstrap` service + wrapper, and the production `JARVIS_API_URL` fix | M12A: scripted fresh install → bootstrap/migrate exit 0 → API+web → proxy `/api/v1/session` 401 → owner bootstrap → browser login, on Windows/Docker Desktop `localhost` (not the Core VM). 16 `tests/deploy/*` + `test_bootstrap_identity.py`. Real-host HTTPS/SSE, scripted restore and security gates remain; orchestrator runtime not started |
